...
def main(msg):
    ...
    if rmse < rmseth or   (time.time() - start_time) > 59.9 :  #len(rmse_data) > iteration:#(time.time() - start_time) > 33.333:
    # if abs(desired_pose[0] - current_pose_x) < 0.0002 and abs(desired_pose[1] - current_pose_y) < 0.0002:
        ...
        # dr =  lmbd*(np.dot(pinv_int_mat_double, dI/0.1)) 
        # vel_input.data = np.dot(pinv_int_manip, dr)

        vel_input.data = lmbd*(np.dot(pinv_int_mat_double, dI/0.1)) 
